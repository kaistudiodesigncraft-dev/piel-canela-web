...
export function TreatmentComboSelector({ treatment }: { treatment: Treatment }) {
  ...
  const selectedExtras = selected?.extras.filter((extra) => selectedExtraIds.includes(extra.id)) ?? [];
  const extraPrice = selectedExtras.reduce((total, extra) => total + extra.priceCents, 0) * (selected?.sessionCount ?? 1);
  const extraDuration = selectedExtras.reduce((total, extra) => total + extra.durationMinutes, 0);
  const selectedPrice = (selected?.fixedPriceCents ?? 0) + extraPrice;
  const selectedDuration = (selected?.durationMinutes ?? 0) + extraDuration + treatment.bufferMinutes;
  ...
  return <section className="combo-selector" aria-labelledby={`combo-selector-${treatment.id}`}>
    ...
    <div className="combo-selector__layout">
      <div className="combo-selector__options" role="radiogroup" aria-label="Combos publicados">
        {visible.length === 0 ? <p className="combo-selector__empty">No hay combos publicados para este filtro.</p> : visible.map((combo) => {
          ...
          return <label key={combo.id} className={`combo-option${checked ? " is-selected" : ""}`}>
            <input type="radio" name={`combo-${treatment.id}`} value={combo.id} checked={checked} onChange={() => { setSelectedId(combo.id); setSelectedExtraIds([]); }} />
      ...
      <aside className="combo-selection-summary" aria-live="polite">
        {selected ? <><span className="status-badge status-confirmed">{labels[selected.audience]}</span><h3>{selected.name}</h3><ul>{selected.zones.map((zone) => <li key={zone.id}><Check aria-hidden="true" />{zone.name}</li>)}</ul>{selected.allowPublicExtras && selected.extras.length > 0 ? <fieldset className="combo-public-extras"><legend>Extras opcionales</legend>{selected.extras.map((extra) => <label key={extra.id}><input type="checkbox" checked={selectedExtraIds.includes(extra.id)} onChange={(event) => setSelectedExtraIds((current) => event.target.checked ? [...current, extra.id] : current.filter((id) => id !== extra.id))} /><span><strong>{extra.name}</strong><small>+{formatDuration(extra.durationMinutes)} · {formatPrice(extra.priceCents)}</small></span></label>)}</fieldset> : null}<dl className="numeric"><div><dt><CalendarClock aria-hidden="true" />Duración por turno</dt><dd>{formatDuration(selectedDuration)}</dd></div><div><dt><Layers3 aria-hidden="true" />Sesiones</dt><dd>{selected.sessionCount}</dd></div>{selected.validityDays ? <div><dt>Vigencia</dt><dd>{selected.validityDays} días</dd></div> : null}<div><dt><CircleDollarSign aria-hidden="true" />Precio total</dt><dd>{formatPrice(selectedPrice)}</dd></div>{selected.mode === "package" ? <div><dt>Por sesión</dt><dd>{formatPrice(Math.round(selectedPrice / selected.sessionCount))}</dd></div> : null}{selected.savingsCents > 0 ? <div className="combo-selection-summary__saving"><dt>Ahorro</dt><dd>{formatPrice(selected.savingsCents)}</dd></div> : null}</dl><Link className="button button--primary" href={buildBookingHref({ treatmentId: treatment.id, comboId: selected.id, extraIds: selectedExtraIds })}>Elegir fecha y horario<ArrowRight aria-hidden="true" /></Link><p className="combo-selection-summary__note">La pre-reserva agenda la primera sesión. El paquete se activa cuando recepción confirma la seña.</p></> : <div className="combo-selection-summary__placeholder"><Layers3 aria-hidden="true" /><h3>Seleccioná una opción</h3><p>Acá vas a ver precio, sesiones, vigencia y el acceso al calendario.</p></div>}
      </aside>
    </div>
  </section>;
}
